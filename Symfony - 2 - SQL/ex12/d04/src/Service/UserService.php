...
class UserService
{
    ...
    public function getPaginatedUsers(int $page, int $limit = 10, string $sort = 'id', string $order = 'asc'): Paginator
    {
        $repository = $this->entityManager->getRepository(User::class);
        $queryBuilder = $repository->createQueryBuilder('u');

        // Set sorting
        if (in_array($sort, ['id', 'username', 'name', 'email', 'birthdate'], true) && in_array(strtolower($order), ['asc', 'desc'], true)) {
            $queryBuilder->orderBy("u.$sort", $order);
        } else {
            $queryBuilder->orderBy('u.id', 'asc'); // Default sorting
        }
        ...
        return new Paginator($query, true); // Wrap the query in Doctrine's Paginator
    }
}
